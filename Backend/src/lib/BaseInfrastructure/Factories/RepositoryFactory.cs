...
{
    public async Task<GenericRepository<TModelType, TKeyType>> CreateRepositoryAsync() => new(await contextFactory.CreateDbContextAsync());
    
    public GenericRepository<TModelType, TKeyType> CreateRepository() => new(contextFactory.CreateDbContext());
}